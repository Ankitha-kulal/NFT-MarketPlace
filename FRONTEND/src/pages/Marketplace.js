...
const Marketplace = () => {
  ...
  // Get unique creators with their profile data
  const allCreators = nfts.reduce((creators, nft) => {
    const creatorData = nft.profiles;
    if (creatorData && creatorData.username) {
      const existingCreator = creators.find(c => c.username === creatorData.username);
      if (!existingCreator) {
        creators.push({
  ...
  // Component for creator avatar with fallback to initials
  const CreatorAvatar = ({ creator, size = "w-12 h-12" }) => {
    ...
    if (!creator.avatar_url || imageError) {
      return (
        <div 
          className={`${size} rounded-full flex items-center justify-center text-white font-bold text-lg ${getRandomColor(creator.username)}`}
        >
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          ...
          <div className="w-full md:w-3/4">
            <div className="relative">
              <input 
                type="text" 
                placeholder={activeTab === 'NFTs' ? "Search NFTs..." : "Search Creators..."} 
          ...
          activeTab === 'NFTs' ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredCreators.map((creator, index) => (
                <div
                  key={index}
                  className={`rounded-xl shadow-md p-6 flex items-center justify-between transition-transform duration-300 hover:shadow-lg hover:transform hover:scale-105 ${
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <span className="text-lg font-medium">{creator.username}</span>
                      <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                        {creator.nftCount} NFT{creator.nftCount !== 1 ? 's' : ''}
                      </p>
                    </div>
                  </div>
        ...
        {!isLoading && activeTab === 'Creators' && filteredCreators.length === 0 && (
          <div className={`text-center py-12 ${darkMode ? 'text-gray-300' : 'text-gray-900'}`}>
            ...
            <p className={`mt-1 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>Try changing your search criteria.</p>
          </div>
        )}
      </div>
    </div>
